...
- **Operator-режим (Паттерн 1, Hermes-desktop):** KROFT_OS вызывается как внешний инструмент через `--query "<cap>:<вопрос>"` (one-shot, без stdin) или pipe в `--interactive`. `--query` добавлен в `composition/run_kroft.py` (KroftConfig.query + main). Пример: `python composition/run_kroft.py --vault "..." --llm auto --query "research:What is the KROFT_OS blackboard pattern?"`.
- **Search Quality v0.1 (2026-08-07):** улучшено ранжирование `ReferenceSearchService` (только веса, без новых слоёв/портов, K5/K6). Причины плохого поиска (ДО): `_overlap` считал релевантность только по телу, игнорируя заголовок; не было буста типа документа (ADR/RFC = conf 0.5 как случайная заметка); запрос «ADR» выдавал шумные заметки с упоминанием «ADR-043» наравне с реальными ADR. Фикс: (A) буст заголовка `_TITLE_BOOST=0.5`, (B) буст типа документа по **имени файла** (`^(adr-|rfc-)` ×1.3, папка architecture ×1.15), (C) исключение стоп-файлов (license/readme/__init__/...) по имени. Результат: запрос «ADR» теперь даёт ADR-001..004 (реальные решения), вытеснив шум; federation/agent loop/desktop имеют чистую градацию ADR(1.95) > ТЗ(1.725) > тело(1.5). Запрос «blackboard pattern» не улучшился — в vault НЕТ документа с «blackboard» в заголовке (дефицит контента, не баг поиска). Тесты: arch 22 / desktop 21 / agent_runtime 20 зелёные.
