...
- **БАГ (исправлен, 2026-08-06):** все 6 агентов (research/architect/finance/planner/programmer/writer) вызывали `self._llm.complete(prompt=...)` — СТРОКУ, хотя порт `ILlm.complete` принимает `ModelQuery` (K1/K6). Вызов падал с `TypeError`, агент ловил `except` и тихо возвращал сырые `graph:`-строки (иллюзия «retrieval работает, синтеза нет»). Фикс: `complete(ModelQuery(prompt=...)).text` во всех 6 агентах + импорт `ModelQuery`. Теперь синтез идёт (проверено на всех 6 capabilities через `--query`).
