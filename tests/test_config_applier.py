...
from contracts.i_optimization import REC_STATUS_APPROVED, REC_STATUS_ROLLED_BACK, Recommendation
from services.config_applier import ConfigApplier


def _rec(rec_id="r1", target="policy:ProviderSelectionPolicy:weights:reasoning", value='{"quality": 0.7}'):
...
def test_full_lifecycle() -> None:
    ...
    assert applier.status(rid) == REC_STATUS_APPROVED
...
def test_history_recorded() -> None:
    ...
    assert hist[0]["new_value"] == {"quality": 0.7}
