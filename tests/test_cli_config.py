...
Verifies that:
  - `init` writes a kroft_os.yaml template,
...
PROJECT = r"C:\Users\Nita\Documents\Obsidian Vault\02-Projects\KROFT_OS" if False else \
    r"C:\Users\Nikita\Documents\Obsidian Vault\02-Projects\KROFT_OS"
...
def test_cmd_init_creates_config():
    ...
    cfg_path = os.path.join(vault, "kroft_os.yaml")
    assert os.path.exists(cfg_path), "init must create kroft_os.yaml"
...
def test_crawl_uses_config_autosave(monkeypatch):
    vault = tempfile.mkdtemp(prefix="hermes-cli-cfg-")
    with open(os.path.join(vault, "kroft_os.yaml"), "w", encoding="utf-8") as fh:
