...
## Product-mode fix: opt-in LLM-синтез через OmniRoute (ТЗ-OMNI-01, ADR-089) — 2026-08-06
- **Проблема:** без LLM агент возвращает сырые `graph:`-строки (pure retrieval). Решение уже было в коде (`composition/omni_router.py` + `build_llm_client(providers=...)`), но НЕ задействовано из `run_kroft.py`.
