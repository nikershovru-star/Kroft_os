"""Planner Agent — planning-specialised agent behaviour (Agents v0.1+, ADR-102).

K5/K6-compliant: services/ imports ONLY contracts.* + stdlib (same seam as the other agents).
Reuses the EXACT proven pattern — no new port, no new layer. Focus: planning / roadmap /
task-decomposition queries resolved against the live knowledge graph.

Behaviour:
    Goal -> Orchestrator.dispatch(capability="planning") -> PlannerAgent.run ->
...
class PlannerAgent(IAgentPlatform):
    """Planning-specialised agent: retrieves roadmap/plan material and answers."""
    ...
    def run(self, goal: str, context: Optional[PolicyContext] = None) -> AgentResult:
        plan = (Step(id="planner", task=goal, status=StepStatus.PENDING),)
        wf = Workflow(
            id=f"planner:{uuid.uuid4().hex[:8]}", goal=goal, plan=plan,
        ...
        if self._llm is not None and hits:
            ...
            if hits:
                ...
                result = result.with_tools("no planning matches in the knowledge graph")
    ...
    def ask(self, goal: str, context: Optional[PolicyContext] = None) -> str:
        ...
        return f"[planner done] {result.status}"


class PlannerAgentExecutor(IAgentExecutor):
    """Bridge PlannerAgent into the Orchestrator's agent dispatch path."""
    ...
    def execute(self, goal: OrchestrationGoal) -> TaskOutcome:
        try:
            ...
            return TaskOutcome(
                success=False, detail=f"planner agent failed: {type(exc).__name__}: {exc}"
            )

    def can_execute(self, goal: OrchestrationGoal) -> bool:
        return goal.capability == "planning"
