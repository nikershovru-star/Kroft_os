"""Writer Agent — writing-specialised agent behaviour (Agents v0.1+, ADR-102).

K5/K6-compliant: services/ imports ONLY contracts.* + stdlib (same seam as the other agents).
Reuses the EXACT proven pattern — no new port, no new layer. Focus: writing / documentation
queries resolved against the live knowledge graph.

Behaviour:
    Goal -> Orchestrator.dispatch(capability="writing") -> WriterAgent.run ->
...
class WriterAgent(IAgentPlatform):
    """Writing-specialised agent: retrieves documentation/material and answers."""
    ...
    def run(self, goal: str, context: Optional[PolicyContext] = None) -> AgentResult:
        plan = (Step(id="writer", task=goal, status=StepStatus.PENDING),)
        wf = Workflow(
            id=f"writer:{uuid.uuid4().hex[:8]}", goal=goal, plan=plan,
        ...
        if self._llm is not None and hits:
            ...
            try:
                synthesis = self._llm.complete(
                    ModelQuery(prompt=f"As a technical writer, draft a concise answer.\n"
                           f"Topic: {goal}\n\nReference material:\n{context_blob}\n\nDraft:")
                ).text
            ...
            if hits:
                ...
                result = result.with_tools("no writing matches in the knowledge graph")
    ...
    def ask(self, goal: str, context: Optional[PolicyContext] = None) -> str:
        ...
        if result.error:
            return f"[writer failed] {result.error}"
        return f"[writer done] {result.status}"


class WriterAgentExecutor(IAgentExecutor):
    """Bridge WriterAgent into the Orchestrator's agent dispatch path."""

    def __init__(self, agent: WriterAgent) -> None:
        self._agent = agent
        self.capability = "writing"
    ...
    def execute(self, goal: OrchestrationGoal) -> TaskOutcome:
        try:
            ...
            return TaskOutcome(
                success=False, detail=f"writer agent failed: {type(exc).__name__}: {exc}"
            )

    def can_execute(self, goal: OrchestrationGoal) -> bool:
        return goal.capability == "writing"
